...
class ComputeLoss:
    ...
    def __init__(
        ...
        self.bbox_loss = BboxLoss(
            self.num_classes, self.reg_max, self.use_dfl, self.iou_type
        ).cuda()
    ...
    def __call__(self, outputs, targets, epoch_num, step_num):
        ...
        if all(
            feat.shape[2:] == cfsize
            for feat, cfsize in zip(feats, self.cached_feat_sizes)
        ):
    ...
    def forward(
        ...
        if num_pos > 0:
            ...
            if self.use_dfl:
                ...
                target_ltrb_pos = torch.masked_select(target_ltrb, bbox_mask).reshape(
                    [-1, 4]
                )
